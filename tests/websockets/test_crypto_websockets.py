...
class CryptoWebsockets:
    ...
    def test_websockets(self):
        ...
        self.price_manager.create_ticker(self.okx_price, override_symbol='BTC-USDT', override_exchange='okx')
        ...
        self.orderbook_manager.create_orderbook(self.okx_orderbook, override_symbol='BTC-USDT',
                                                override_exchange='okx')
    ...
    def okx_price(self, message):
        if self.validate_price_event(message):
            self.validated_responses['prices']['okx'] = True
            self.price_manager.close_websocket('BTC-USDT', 'okx')
    ...
    def okx_orderbook(self, message):
        if self.validate_orderbook_event(message):
            self.validated_responses['orderbook']['okx'] = True
            self.orderbook_manager.close_websocket('BTC-USDT', 'okx')
