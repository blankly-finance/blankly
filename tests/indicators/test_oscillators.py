...
class Oscillators(unittest.TestCase):
    ...
    def test_rsi(self):
        for period in self.data['periods']:
            ...
            self.assertTrue(rsi_res, self.data[period]['rsi'])

    def test_aroon_oscillator(self):
        for period in self.data['periods']:
            aroon_res = rsi(self.data['high'], self.data['low'], period)
            self.assertTrue(aroon_res, self.data[period]['aroon_oscillator'])

    def test_chande_momentum_oscillator(self):
        for period in self.data['periods']:
            chande_res = chande_momentum_oscillator(self.data['close'], period)
            self.assertTrue(chande_res, self.data[period]['chande_momentum_oscillator'])
    ...
    def test_absolute_price_oscillator(self):
        ...
        self.assertTrue(res, self.data[self.data['periods'][0]]['absolute_price_oscillator'])
    ...
    def test_stochastic_oscillator(self):
        ...
        res = stochastic_oscillator(self.data['close'], pct_k_period, pct_k_slowing_period, pct_d_period)
        # use the first one since general period doesn't matter
        self.assertTrue(res, self.data[self.data['periods'][0]]['stochastic_oscillator'])